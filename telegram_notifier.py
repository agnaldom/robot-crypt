...
# Desativando avisos de SSL para evitar mensagens de erro repetitivas nos logs
...
class TelegramNotifier:
    ...
    def __init__(self, bot_token, chat_id):
        ...
        # Aviso sobre a desativação da verificação SSL
        self.logger.warning("⚠️ Verificação SSL desabilitada nas requisições para o Telegram")
        self.logger.warning("⚠️ Esta é uma solução temporária para o erro de certificado")
    ...
    def send_message(self, message):
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    # Desabilitando a verificação SSL temporariamente para resolver o problema de certificado
                    response = requests.post(url, data=data, timeout=30, verify=False)
                    # Aviso sobre segurança
                    self.logger.warning("Aviso: Verificação SSL desabilitada para requisições ao Telegram")
