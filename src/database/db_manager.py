...
class DBManager:
    ...
    def __init__(self, db_path="data/robot_crypt.db"):
        ...
        self.connect()
        self.setup_tables()
        self.ensure_simulation_mode_column()  # Verifica e adiciona coluna simulation_mode se necessária
    ...
    def update_daily_stats(self, stats):
        ...
        try:
            ...
            default_stats = {
                ...
                'simulation_mode': 0       # Valor padrão para modo de simulação (0=real, 1=simulação)
            }
            ...
            # Verificar se temos a informação do modo de simulação
            if 'simulation_mode' not in stats:
                # Tentar obter da configuração
                try:
                    from config import Config
                    config = Config()
                    stats['simulation_mode'] = 1 if config.simulation_mode else 0
                    self.logger.info(f"Modo de simulação definido como: {stats['simulation_mode']}")
                except Exception:
                    self.logger.warning("Não foi possível obter configuração de simulação, usando valor padrão: 0")
                    stats['simulation_mode'] = 0
            ...
            if result:
                # Verifica se a coluna simulation_mode existe
                try:
                    ...
                    # Se a coluna não existir, atualizar sem ela
                    self.logger.warning(f"Erro ao atualizar coluna simulation_mode: {str(e)}")
                    self.cursor.execute('''
                        UPDATE stats SET
                        final_capital = ?,
                        total_trades = ?,
                ...
                # Verifica se a coluna simulation_mode existe
                try:
                    # Insere novas estatísticas com campo simulation_mode
                    self.cursor.execute('''
                        INSERT INTO stats (date, initial_capital, final_capital, total_trades, 
                                        winning_trades, losing_trades, best_trade_profit, worst_trade_loss, simulation_mode)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        today,
                    ...
                    self.logger.warning(f"Erro ao inserir coluna simulation_mode: {str(e)}")
                    self.cursor.execute('''
                        INSERT INTO stats (date, initial_capital, final_capital, total_trades, 
    ...
    def ensure_simulation_mode_column(self):
        """Verifica e adiciona a coluna simulation_mode à tabela stats se necessária"""
        try:
            # Verificar se a coluna simulation_mode existe na tabela stats
            self.cursor.execute("PRAGMA table_info(stats)")
            columns = self.cursor.fetchall()
            column_names = [col[1] for col in columns]
            
            if 'simulation_mode' not in column_names:
                self.logger.info("Adicionando coluna simulation_mode à tabela stats")
                self.cursor.execute("ALTER TABLE stats ADD COLUMN simulation_mode INTEGER DEFAULT 0")
                self.conn.commit()
                ...
                try:
                    ...
                    self.logger.info(f"Definindo registros existentes com modo de simulação: {simulation_mode}")
                except Exception as e:
                    self.logger.warning(f"Não foi possível obter configuração: {str(e)}")
                
                # Atualizar registros existentes
                self.cursor.execute("UPDATE stats SET simulation_mode = ? WHERE simulation_mode IS NULL", (simulation_mode,))
                self.conn.commit()
                self.logger.info(f"Coluna simulation_mode adicionada com sucesso. Atualizados {self.cursor.rowcount} registros.")
                return True
            else:
                self.logger.debug("Coluna simulation_mode já existe na tabela stats")
                return True
        except Exception as e:
            self.logger.error(f"Erro ao verificar/adicionar coluna simulation_mode: {str(e)}")
            return False
