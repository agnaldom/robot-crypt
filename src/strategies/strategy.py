...
class ScalpingStrategy(TradingStrategy):
    ...
    def identify_support_resistance(self, symbol, period="1h", lookback=24):
        ...
        try:
            ...
            api_symbol = format_symbol(symbol)  # Remove a barra e sanitiza o símbolo
            ...
            if not klines or len(klines) == 0:
                self.logger.warning(f"Nenhum dado de velas retornado para {symbol} ({api_symbol})")
                return None
            ...
            try:
                ...
                self.logger.error(f"Erro ao processar dados de velas para {symbol}: {str(e)}")
                return None
