...
class BinanceAPI:
    ...
    def get_klines(self, symbol, interval, limit=500):
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response and e.response.status_code == 400:
                self._log_structured("warning", 
                    f"Símbolo {symbol} ({formatted_symbol}) não é válido na Binance", 
                    {
                ...
                # Retorna array vazio para que o código continue funcionando
                return []
            else:
    ...
    def validate_trading_pairs(self, pairs):
        """Valida os pares de trading fornecidos com base nas informações de troca da Binance."""
        exchange_info = self.get_exchange_info()
        valid_symbols = {s['symbol']: s for s in exchange_info.get('symbols', [])}

        valid_pairs = []
        for pair in pairs:
            if pair.replace("/", "") in valid_symbols:
                valid_pairs.append(pair)
            else:
                self.logger.warning(f"Par de trading inválido: {pair} não existe na Binance")
    ...
    def get_recent_orders(self, symbol, limit=10):
        """
        Obtém ordens recentes para um determinado símbolo
        
        Args:
            symbol (str): Símbolo do par (ex: BTCUSDT)
            limit (int): Número máximo de ordens para retornar (default: 10)
            
        Returns:
            list: Lista de ordens recentes
        """
        ...
        self._log_structured("debug", "Consultando ordens recentes", 
                            {"endpoint": endpoint, "symbol": symbol, "limit": limit})
        
        try:
            result = self._make_request('GET', endpoint, params, signed=True)
            return result
        except Exception as e:
            self._log_structured("error", "Falha ao obter ordens recentes", 
                               {"endpoint": endpoint, "symbol": symbol}, e)
            return []
    
    def validate_trading_pair(self, symbol):
        """
        Verifica se um par de trading é válido na Binance
        
        Args:
            symbol (str): Par de trading no formato 'BTC/USDT' ou 'BTCUSDT'
            
        Returns:
            dict: Informações do par se for válido, None caso contrário
            
        Raises:
            ValueError: Se o par não existir ou não estiver disponível para trading
        """
        self.logger.debug(f"Validando par de trading: {symbol}")
        
        # Formata o símbolo para o formato esperado pela Binance (sem a /)
        ...
        try:
            ...
            if formatted_symbol not in symbols_info:
                self.logger.warning(f"Par de trading inválido: {symbol} não existe na Binance")
                return None
                
            # Verifica se o símbolo está disponível para trading
            symbol_info = symbols_info[formatted_symbol]
            if symbol_info['status'] != 'TRADING':
                self.logger.warning(f"Par de trading {symbol} não está disponível para trading. Status: {symbol_info['status']}")
                return None
    ...
    def get_valid_trading_pairs(self, base_assets=None, quote_assets=None):
        """
        Obtém todos os pares de trading válidos, opcionalmente filtrados por moedas base ou quote
        
        Args:
            base_assets (list): Lista de moedas base para filtrar (ex: ['BTC', 'ETH'])
            quote_assets (list): Lista de moedas quote para filtrar (ex: ['USDT', 'BTC'])
            
        Returns:
            list: Lista de pares de trading válidos no formato 'BTC/USDT'
        """
        self.logger.debug("Obtendo pares de trading válidos")
        ...
        try:
            ...
            for symbol_info in exchange_info.get('symbols', []):
                ...
                if base_assets and base_asset not in base_assets:
                    continue
                if quote_assets and quote_asset not in quote_assets:
                    continue
                
                valid_pairs.append(f"{base_asset}/{quote_asset}")
                
            self.logger.info(f"Encontrados {len(valid_pairs)} pares de trading válidos")
            return valid_pairs
        except Exception as e:
            self.logger.error(f"Erro ao obter pares de trading válidos: {str(e)}")
            return []
