...
while true; do
    ...
    echo "11. Manutenção do banco PostgreSQL"
    echo "12. Sair"
    echo
    read -p "Escolha uma opção (1-12): " option
    echo
    ...
    case $option in
        ...
        11) 
            echo -e "${BLUE}=========================================================${NC}"
            echo -e "${BLUE}          Manutenção do Banco PostgreSQL                 ${NC}"
            echo -e "${BLUE}=========================================================${NC}"
            echo
            echo -e "${YELLOW}Escolha uma opção de manutenção PostgreSQL:${NC}"
            echo "1. Verificar status das tabelas"
            echo "2. Preencher tabela system_health"
            echo "3. Calcular indicadores técnicos"
            echo "4. Gerar análises de mercado"
            echo "5. Gerar métricas de performance"
            echo "6. Gerar operações de exemplo"
            echo "7. Executar todas as tarefas"
            echo "8. Voltar ao menu principal"
            echo
            read -p "Escolha uma opção (1-8): " pg_option
            echo
            
            case $pg_option in
                1) python3 maintenance_tool.py --check ;;
                2) python3 maintenance_tool.py --health ;;
                3) python3 maintenance_tool.py --indicators ;;
                4) python3 maintenance_tool.py --analysis ;;
                5) python3 maintenance_tool.py --metrics ;;
                6) python3 maintenance_tool.py --trades ;;
                7) python3 maintenance_tool.py --all ;;
                8) echo "Voltando ao menu principal..." ;;
                *) echo -e "${RED}Opção inválida${NC}" ;;
            esac
            ;;
        12) echo "Encerrando..."; exit 0 ;;
        *) echo -e "${RED}Opção inválida${NC}" ;;
    esac
